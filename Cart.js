    ...
    {Array.isArray(data) && data.length > 0 ?
      ...
      {data.map((el,i)=>(
         ...
         <div class="card-body">
           ...
           {/*<a href="#"class="btn btn-primary"  style={{marginRight : 10}}>View More</a>
          ...
          <button  disabled={el.disable == true ? true : false} onClick={()=>{addToCart(el)}} class="btn btn-success">{  el.disable == true ? "Already Added" : 'Add To Cart'}</button>
           <button  disabled={el.disable == true ? true : false} onClick={()=>{addToCart(el)}} class="btn btn-success"></button>*/}
